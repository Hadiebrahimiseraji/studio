...
3.  **مدیریت فایل‌ها و تصاویر:**
    -   در فایل `settings/base.py`، متغیر `MEDIA_ROOT` طوری تنظیم شده که به پوشه‌ی `finalthesis/database/image/` اشاره کند.
...
3.  **اتصال به بک‌اند (API Calls):**
    ...
    -   تابع `getFullImageUrl()` مسیر نسبی تصویر دریافتی از بک‌اند (مثلاً `/media/database/image/1/p-13877.jpg`) را به یک URL کامل تبدیل می‌کند تا در تگ `<img>` قابل استفاده باشد.
