...
1.  **مدیریت پایگاه داده:**
    -   مدل‌های داده (مانند `Product`, `Category`) در فایل‌ `apps/catalog/models.py` تعریف شده‌اند. این مدل‌ها ساختار جداول پایگاه داده را مشخص می‌کنند.
...
2.  **ارائه API RESTful:**
    ...
    -   **Serializerها** (`apps/catalog/serializers.py`) مسئول تبدیل مدل‌های پیچیده‌ی پایتون به JSON هستند.
    -   **Viewها** (`apps/catalog/views.py`) منطق هر API را مدیریت می‌کنند (مثلاً دریافت تمام محصولات یا فیلتر بر اساس دسته‌بندی).
    -   **URLها** (`apps/catalog/urls.py`) مسیرهای API را تعریف می‌کنند (مثلاً `/api/catalog/products/`).

3.  **مدیریت فایل‌ها و تصاویر:**
    -   در فایل `settings/base.py`، متغیر `MEDIA_ROOT` طوری تنظیم شده که به پوشه‌ی `finalthesis/database/` اشاره کند. این به Django اجازه می‌دهد تا تصاویر محصولات را مستقیماً از این پوشه به فرانت‌اند سرویس دهد.
...
1.  **ساختار صفحات (App Router):**
    ...
    -   `app/products/page.tsx`: صفحه‌ی لیست تمام محصولات (`/products`). این صفحه قابلیت فیلتر بر اساس دسته‌بندی را از طریق query-string دارد (مثلاً `/products?category=...`).
    -   `app/products/[id]/page.tsx`: صفحه‌ی نمایش جزئیات یک محصول خاص. `[id]` یک پارامتر داینامیک است.

2.  **کامپوننت‌ها (Components):**
    -   در پوشه‌ی `src/components/`، قطعات قابل استفاده مجدد UI قرار دارند. برای مثال `ProductCard.tsx` یک کارت زیبا برای نمایش خلاصه‌ی محصول است.
...
5.  **استایل‌دهی (Tailwind CSS):**
    -   به جای نوشتن کدهای CSS سنتی، از کلاس‌های آماده‌ی Tailwind برای استایل‌دهی سریع و واکنش‌گرا (Responsive) استفاده می‌شود.

---

## ۴. راهنمای راه‌اندازی پروژه (قدم به قدم برای مبتدیان)
...
5.  **پایگاه داده را ایجاد کنید (Migration):**
    این دستور جداول مورد نیاز برنامه (مانند جدول محصولات و دسته‌بندی‌ها) را در یک فایل پایگاه داده SQLite ایجاد می‌کند.
    ```bash
    python manage.py migrate
    ```

6.  **داده‌های اولیه را وارد پایگاه داده کنید (بسیار مهم):**
    این دستور، دسته‌بندی‌ها و محصولات نمونه را از فایل `sample_products.json` خوانده و به صورت خودکار به پایگاه داده اضافه می‌کند. **هر بار که خواستید داده‌ها را از نو بسازید، این دستور را اجرا کنید.**
    ```bash
